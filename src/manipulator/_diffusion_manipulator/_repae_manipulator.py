...
class REPAEManipulator:
    ...
    def __init__(
        ...
        self._latents_scale = state_dict['ema']["bn.running_var"].rsqrt().view(1, self._in_channels, 1, 1).to(self._device)
        self._latents_bias = state_dict["ema"]["bn.running_mean"].view(1, self._in_channels, 1, 1).to(self._device)
        ...
        self._embed_y = lambda y: self._model.y_embedder(torch.tensor(y, device=self._device), self._model.training)
    ...
    def manipulate(
        ...
        assert candidates.shape[:1] == weights.shape, f"ERROR, candidates have shape {candidates.shape}, weights have shape {weights.shape}"
        assert candidate_ys.shape[:1] == weights_y.shape, f"ERROR, candidates have shape {candidate_ys.shape}, weights have shape {weights_y.shape}"

        x_manip = torch.sum(candidates * weights[:,None,None,None], dim=0, keepdim=True)  # N x X -> 1 x X
        y_manip = torch.sum(candidate_ys * weights_y[:,None], dim=0, keepdim=True)  # N x Y -> 1 x Y
    ...
    def _sample(
            ...
            step: float,
            cfg_scale:float=1.5,
            guidance_bounds: tuple[float, float]=(0., 1.)
    ) -> Tensor:
        """
        ...
        with (torch.no_grad()):
            ...
            if cfg_scale > 1. and guidance_bounds[1] >= t >= guidance_bounds[0]:
    ...
    def get_diff_steps(self, class_labels: list[int], n_steps: int=50) -> tuple[list[Tensor], Tensor]:
        """
        ...
        """
        x_cur = torch.randn(len(class_labels), self._in_channels, self._latent_size, self._latent_size, device=self._device)
        xs = [x_cur]
    ...
    def finish_diffusion(self, x_cur:Tensor, y_cur: Tensor, t_cur_index:int, t_steps: Tensor, return_all:bool = False) -> Tensor:
        """
    ...
    def get_image(self, z: Tensor) -> Tensor:
        ...
        element = (element + 1) / 2.
        element = torch.clamp(255. * element, 0, 255).permute(0, 2, 3, 1).type(torch.uint8)
        return element
